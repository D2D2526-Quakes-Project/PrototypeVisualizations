...
def discover_buildings():
    ...
    for building_folder in os.listdir(CSV_DIR):
        ...
        if os.path.isdir(building_path):
            ...
            if os.path.exists(node_data_file) and os.path.exists(height_file):
...
def get_simulation_files(building_folder, simulation):
    ...
    files: dict[str, dict[str, list[str] | None] | None] = {"displacement": None, "velocity": None, "acceleration": None, "ground_motion": None}
    ...
    if simulation["has_displacement"]:
        ...
        if pattern == "Entire":
            ...
            # For Grid pattern, we need to determine which grid files exist
            # Use Grid_11 as default, fallback to Grid_36 if needed
            grid_num = "11"
            if not os.path.exists(os.path.join(disp_path, f"D_H1_Grid_{grid_num}.txt")):
                grid_num = "36"

            files["displacement"] = {
                "lin": [
                    os.path.join(disp_path, f"D_H1_Grid_{grid_num}.txt"),
                    os.path.join(disp_path, f"D_H2_Grid_{grid_num}.txt"),
                    os.path.join(disp_path, f"D_V_Grid_{grid_num}.txt"),
                ],
                "rot": None,  # Grid files don't have rotation components
            }
    ...
    if simulation["has_ground_motion"]:
        files["ground_motion"] = simulation.get("ground_motion_file")
...
def process_response_file(file_key, type_name, id_to_index, files_config, simulation_output_dir):
    ...
    Handles both 6-file (with rotation) and 3-file (linear only) formats.
    """
    file_list = files_config.get(file_key)
    if not file_list:
        print(f"Skipping {type_name}: Files not available.")
        return
    ...
    # Stack: (Frames, Nodes, 6) or (Frames, Nodes, 3)
    if components_per_node == 6:
        stacked = np.stack([aligned_lx, aligned_ly, aligned_lz, aligned_rx, aligned_ry, aligned_rz], axis=2)
    else:
        stacked = np.stack([aligned_lx, aligned_ly, aligned_lz], axis=2)

    # Flatten: Frame 0 (Node 0..N), Frame 1...
    buffer = stacked.flatten()

    # 4. Write
    header = {"type": type_name, "count_frames": num_frames, "count_nodes": num_nodes, "dt": 0.01, "has_rotation": has_rotation}

    write_bld_file(f"{file_key}.bld", header, buffer.tobytes(), simulation_output_dir)
...
def process_complete_building(building):
    ...
    try:
        ...
        return (building["name"], "error", str(e))
...
if __name__ == "__main__":
    ...
    print("Discovering buildings and simulations...")
