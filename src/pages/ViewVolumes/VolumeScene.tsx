...
export function VolumeScene({
  ...
  for (let i = 0; i < storyOrder.length - 1; i++) {
    ...
    const currentCornerIndices = currentNodeIndices.filter((idx) => 
      cornerSets.NW.has(idx) || cornerSets.NE.has(idx) || 
      cornerSets.SW.has(idx) || cornerSets.SE.has(idx)
    );
    
    const nextCornerIndices = nextNodeIndices.filter((idx) => 
      cornerSets.NW.has(idx) || cornerSets.NE.has(idx) || 
      cornerSets.SW.has(idx) || cornerSets.SE.has(idx)
    );
    ...
    let currentTotalDx = 0, currentTotalDy = 0, currentTotalDz = 0;
