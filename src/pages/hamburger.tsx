...
export function ViewHamburger() {
  ...
  const totalDisplacementForFrame = storyData.reduce((acc, story) => acc + story.avgDispMag, 0);
    ...
    <div className="flex h-full flex-col gap-2">
      <div>
        <span>
          ...
          {Math.hypot(...avgBuildingDisplacement)?.toFixed(2)}in
        </span>
        <br />
        <span>
          Color is the average displacement of the story scaled by maximum displacement. The size is a percentage of the
      ...
      <div className="flex gap-1">
        {storyData.map((story, i) => {
          ...
          const widthPercent = totalDisplacementForFrame > 0 ? (story.avgDispMag / totalDisplacementForFrame) * 100 : 0;
      ...
      <div className="flex h-full flex-col gap-1">
        {storyData.map((story, i) => {
          const color = formatHex(colorMap(story.avgDispMag / maxDisplacement));
          const heightPercent =
            totalDisplacementForFrame > 0 ? (story.avgDispMag / totalDisplacementForFrame) * 100 : 0;
          return (
            <div key={i} className="w-full rounded" style={{ backgroundColor: color, height: `${heightPercent}%` }} />
          );
        })}
      </div>
    </div>
